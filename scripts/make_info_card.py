...
ROWS = [
    ("host",),
    ("kv", "Now", "Co-Founder & AI Engineer @ Brezix Studio"),
    ...
    ("gap",),
    ("sec", "Stack"),
    ("kv", "AI / ML", "LangChain, LangGraph, RAG, AI Agents"),
    ("kv", "Backend", "FastAPI, Python, REST APIs"),
    ("kv", "Retrieval", "ChromaDB, FAISS, Embeddings"),
    ("kv", "Frontend", "React, Next.js"),
    ("kv", "Cloud", "Docker, Git, Vercel, Supabase"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "Built SnapClass — face + voice AI attendance system"),
    ("bul", "Built IntervuAI — LLM-powered mock interview coach"),
    ("bul", "Builds in public — AI/ML journey on LinkedIn"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">akshay@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">akshay</tspan><tspan fill="{MUTED}">@</tspan>'
